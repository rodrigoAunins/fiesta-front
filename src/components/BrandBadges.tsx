...
export function SecureLockBadge({ size = 'md' }: { size?: Size }) {
  const box = size === 'sm' ? 'h-10 w-10 rounded-2xl text-base' : 'h-12 w-12 rounded-2xl text-lg';

  return (
    <div className={`flex ${box} items-center justify-center bg-[#e8fff2] text-[#00a650] shadow-sm`}>
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="leading-tight">
        ...
        <p className="text-xs text-slate-500">Cobro integrado</p>
      </div>
    </div>
  );
}
